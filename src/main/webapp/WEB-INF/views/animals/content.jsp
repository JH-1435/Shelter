...
<div id="#container-wrap">
	...
	<div class="a-table view02">
		<div>
		<h4 class="content-h4">유기동물 정보</h4>
		<!-- submit 하면 form 값을 @GM("/orders")로 보냄 -->
			<form method="get" action="order" >
			<table>
			<caption>입양선택</caption>
			<colgroup>
				<col style="width:20%;">
			...
			<tbody>
				<c:forEach var="item" items="${sessionScope.content}">
					...
					<c:if test="${sessionScope.content == null}">
						<tr><td colspan="4">선택한 동물이 없습니다.</td></tr>
					</c:if>
					...
					<tr>
						<td colspan="2">
							...
							<ul>
								<li>
									<img src="/ShelterUpload/${item.value.animalImg}"  class="photo-img"
									alt="${item.value.animalImg}">
								</li>
							</ul>
							</div>
							</div>
					...
					<tr>
						...
						<td class="av-1">${item.key}</td>
					</tr>
					<tr>
						<th colspan="1" class="av-1">종류</th><td class="av-1">${item.value.animalTitle}</td>
					</tr>
					<tr>
						<th colspan="1" class="av-1">품종</th><td class="av-1">${item.value.animalName}</td>
					</tr>
					<tr>
						<th colspan="1" class="av-1">성별</th><td class="av-1">${item.value.animalSex}</td>
					</tr>
					</c:forEach>
				<tr>
					<td colspan="2"><h4 class="content-h4">입양신청자 정보입력</h4></td>
				</tr>
				<tr class="content-user-tr">
					<td colspan="5">${sessionScope.user.userName}(${sessionScope.user.userId})</td>
				</tr>
				<tr class="content-text">
					<th><label>신청제목(*)</label></th>
					<td colspan="1"><input name="boardTitle" type="text" class="content-text-box"></td>
				</tr>
				<tr class="content-text">
					<th><label>내용(*)</label></th>
					<td colspan="1"><textarea rows="6" cols="72" name="boardContent" maxlength="2048"></textarea>
					</td>
				</tr>		
				<tr class="content-submit">
					<td colspan="5"><input type="submit" value="신청완료"></td>
				</tr>
				</tbody>
			</table>
			</form>
		...
		<div class="btn-group">
			<a href="list"><i class="fas fa-bars"></i>추가 신청</a>
			<a href="contentInit"><i class="fas fa-bars"></i>취소</a>
		</div>	
		</div>
	</div>
	<!---- 입양선택 ---->
